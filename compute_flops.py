...
def main():
    ...
    if args.prune_cfg == 1:
        ...
        print_model_param_flops(network, input_res=32)
        print_model_param_nums(network)
        exit()
    # Doesn't matter which method we use to prune
    network = prune(network, prune_layers, prune_channels, magnitude=True)
    print_model_param_flops(network, input_res=32)
    print_model_param_nums(network)
